...
const usernameArgs = {
  where: { username: 'admin', connected: true }
}
...
test.beforeEach(async () => {
  ...
  AgentStub.findAll.withArgs(usernameArgs).returns(Promise.resolve(agentFixtures.admin))
...
test.serial('Agent#findByUsername', async t => {
  const agents = await db.Agent.findByUsername('admin')

  t.true(AgentStub.findAll.called, 'findAll should be called on model')
  t.true(AgentStub.findAll.calledOnce, 'findAll should be called once')
  t.true(AgentStub.findAll.calledWith(usernameArgs), 'findAll should be called with username args')

  t.is(agents.length, agentFixtures.admin.length, 'agents should be the same amount')
  t.deepEqual(agents, agentFixtures.admin, 'agents should be the same')
})
